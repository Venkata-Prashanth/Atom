...
double radius = 0.195;                              //Wheel radius, in m
double wheelbase = 0.267;                           //Wheelbase, in m
double two_pi = 6.28319;
double speedActualLeft = 0.0;
double speedActualRight = 0.0;
...
void handle_speed( const geometry_msgs::Vector3Stamped& speed) {
  speedActualLeft = trunc(speed.vector.x*100)/100;
  ROS_INFO("speed left : %f", speedActualLeft);
  speedActualRight = trunc(speed.vector.y*100)/100;
  ROS_INFO("speed right : %f", speedActualRight);
  speed_dt = speed.vector.z;
  speed_time = speed.header.stamp;
}

int main(int argc, char** argv){
  ros::init(argc, argv, "atom_controller");
  ...
  while(n.ok()){
    ...
    ROS_INFO("dt : %f", dt);
    dxy = (speedActualLeft+speedActualRight)*dt/2;
    ROS_INFO("dxy : %f", dxy);
    dth = ((speedActualRight-speedActualLeft)*dt)/wheelbase;
    ...
    if (speedActualLeft == 0 && speedActualRight == 0){
    ...
    vx = (dt == 0)?  0 : (speedActualLeft+speedActualRight)/2;
    vth = (dt == 0)? 0 : (speedActualRight-speedActualLeft)/wheelbase;
